...
class ExperimentRun(pytry.NengoTrial):
	...
	def model(self, p):
		...
		model = Model(p.vocab, p.xp, p.s_crosstalk, p.s_evidence, p.n_samples, p.integrator_reset, p.t_senso, p.proc_feedback, p.proc_feedback_synapse, p.GW_feedback, p.GW_threshold, p.GW_scale, p.BG_thr, p.BG_bias, n_neurons_per_dim, p.seed, p.plt)
		model.make_probes()
...
class Model():
	...
	def construct_network(self):
		...
		with net:
			...
			net.PREV = spa.State(self.vocab, feedback=.9, feedback_synapse=.02, label='PREV')
