...
import numpy as np
from modules import Processor, Button, DirectCompareProcessor, Delay
...
class ExperimentRun(pytry.NengoTrial):
	...
	def model(self, p):
		...
		model = Model(p.vocab, p.xp, p.s_crosstalk, p.s_evidence, p.t_senso, p.proc_feedback, p.GW_feedback, p.GW_scale, p.BG_thr, p.BG_bias, n_neurons_per_dim, p.seed)
		model.make_probes()
...
class Model():
	def __init__(self, vocab, experiment, s_crosstalk, s_evidence, t_senso, proc_feedback, GW_feedback, GW_scale, BG_thr, BG_bias, n_neurons_per_dim, seed=None):
		self.vocab = vocab
	...
	def construct_network(self):
		...
		with net:
			...
			nengo.Connection(net.input_net.senso_delay, net.V.input, synapse=None)
